...
def test(model, data):
    ...
    image = img * 255
    Image.fromarray(image.astype(np.uint8)).save("real_and_recon_mnist_rot.png")
    print()
    print('Reconstructed images are saved to ./real_and_recon_mnist_rot.png')
    print('-'*50)
    plt.imshow(plt.imread("real_and_recon_mnist_rot.png", ))
    plt.show()
...
def load_mnist():
    ...
    for i in xrange(0, x_test.shape[0]):
        #x_test[i] = np.rot90(np.rot90(x_test[i]))
